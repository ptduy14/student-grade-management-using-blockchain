...
import {
  TeacherSignInValidationSchemaType,
  teacherSignInValidationSchema,
} from "./schema/teacher-sigin-validation-schema";
...
export const TeacherSigninForm = ({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<TeacherSignInValidationSchemaType>({
    resolver: zodResolver(teacherSignInValidationSchema),
  });
  const { signIn } = useAuth();
  const user = useSelector((state: any) => state.account.user);

  const onSubmit: SubmitHandler<TeacherSignInValidationSchemaType> = async (
    data
  ) => {
    setIsLoading(true);
    await signIn(data);
        ...
        <div className="mx-auto max-w-xs">
          ...
          <span className="block mt-6 text-sm text-gray-600 text-center">
            ...
            <span
              ...
              onClick={() => setIsTeacherSigninForm(false)}
            >
              sinh viên
            </span>
          </span>
